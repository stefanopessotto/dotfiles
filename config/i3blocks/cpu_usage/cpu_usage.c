...
void display(const char *label, double used, int const warning,
             int const critical, int const decimals) {
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    display(label, 100.0D * (used - old_used) / (total - old_total), warning,
            critical, decimals);
    fflush(stdout);
    old_total = total;
    old_used = used;
